...
const hsk3New = JSON.parse(fs.readFileSync('scripts/hsk3-new.json', 'utf8'));

const hsk1Corrections = new Set([
  '休息', '雪', '要', '也', '元', '再', '早上', '找', '真', '正在', '知道',
]);

// Former supplementary (non-list) words that are official New HSK 3 vocabulary.
const hsk3Corrections = new Set([
  '帮助', '报纸', '船', '公斤', '欢迎', '回答', '男人', '女人', '为',
  '西瓜', '向', '张', '自行车',
]);

const supplementaryWords = new Set([
  '火车站', '北京', '小姐', '没', '打篮球', '公共汽车', '羊肉',
  '服务员', '唱歌', '白',
  '红', '黑', '第一', '星期一',
  '星期二', '星期三', '星期四', '星期五', '星期六',
]);
...
const newWords = [...hsk1New, ...hsk2New, ...hsk3New].filter(w => !existingHz.has(w.hz));
...
console.log('Skipped duplicates:', hsk1New.length + hsk2New.length + hsk3New.length - newWords.length);
...
const newNodes = newWords.map((w) => {
  return {
    ...
    hsk: w.hsk,
    pos: w.pos,
    // Filled from SUBTLEX-CH by scripts/update-frequency-data.mjs.
    freq: null,
    freqCount: null,
    freqPerMillion: null,
    isHub: false,
...
for (const word of allWords) {
  ...
  if (hsk3Corrections.has(word.hz)) word.hsk = 3;
...
const h3 = allWords.filter(w => w.hsk === 3).length;
data.meta = {
